...
$(document).ready(function () {
	//history.forward(1);
    $('input[type=text],input[type=password],textarea,.hindi-marathi').not('.ckeditor').on('keyup', function () {
    ...
    //FUnction to disable speacial characters on keyup :by: T Ketaki Debadarshini 
     $('input[type=text],textarea').not('.ckeditor,.hindi-marathi,#txtUserName').on('keypress', function (e) {
            var key = e.keyCode || e.which;
            var regex = new RegExp("^[a-zA-Z0-9\-_,.@+:'%)(;/\n ]*$");
            var str = String.fromCharCode(!e.charCode ? e.which : e.charCode);
            if (regex.test(str) || key === 9 || key === 13 ||  key === 8 || key ===37 || key === 38 ||  key === 39 ||  key === 40 ||  key === 59) {
                return true;
            }
            return false;
...
function maxLength(controlId, ctrlLen, fieldName)
{
    ...
    {
        viewAlert(fieldName + ' exceeds ' + ctrlLen + ' characters !!!', controlId);
        $('#' + controlId).focus();
        return false;
    }
    return true;
...
function isNumber(controlId)
{
    ...
    if(data != ' ')
    {
        ...
        {
            if(data != ''){
                viewAlert("Enter only number", controlId);
                $('#' + controlId).focus();
                return false;
            }else{
                return true;
