...
function checkInputDecimal(evt, el)
{
	var charCode = (evt.which) ? evt.which : event.keyCode;
    var number = el.value.split('.');
    if (charCode != 46 && charCode > 31 && (charCode < 48 || charCode > 57)) {
...
//=============== Function to compare date BY :: manali ranjan on :: 09-04-2019==============
function compareEqualDate(firstControlDate, secondControlDate, field1NameDate, field2NameDate,firstControlTime, secondControlTime, field1NameTime, field2NameTime,msg)
{
//alert(typeof(msg));
    var firstDate = $('#' + firstControlDate).val();
    var secondDate = $('#' + secondControlDate).val();
    var firstTime = $('#' + firstControlTime).val();
    var secondTime = $('#' + secondControlTime).val();
    //alert(firstTime+'==='+secondTime);
    var splitFirstDt = firstDate.split("-");
    firstDate = splitFirstDt[2] + '-' + splitFirstDt[1] + '-' + splitFirstDt[0];
    var splitSecondDt = secondDate.split("-");
    secondDate = splitSecondDt[2] + '-' + splitSecondDt[1] + '-' + splitSecondDt[0];
    var date2=firstDate+' '+firstTime;
    var date1=secondDate+' '+secondTime;
    var firstDateNew = new Date(date2);
    var secondDateNew = new Date(date1);
    if (firstDateNew.getTime() === secondDateNew.getTime())
    {
        
        if (typeof msg == "undefined"){
            viewAlert(field1NameTime + " cannot be same as " + field2NameTime, firstControlTime);
             return false;
        }
        else
            viewAlert(msg, firstControlTime);
            $('#' + secondControlTime).focus();
        return false;
    }
    else{
        return true;
...
//=============== Function to compare date and focus on second control==============
function compareDateRep(firstControl, secondControl, field1Name, field2Name, msg)
{
//alert(typeof(msg));
    var firstDate = $('#' + firstControl).val();
    var secondDate = $('#' + secondControl).val();
